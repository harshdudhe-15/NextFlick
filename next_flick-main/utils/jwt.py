...
class JwtMiddleware(BaseHTTPMiddleware):
    def __init__(self, app, exempt_routes=None, prefix_exempt_routes=None):
        super().__init__(app)
        self.exempt_routes = exempt_routes or [] 
        self.prefix_exempt_routes = prefix_exempt_routes or []
    ...
    async def dispatch(self, request, call_next):
        ...
        if any(path.startswith(route) for route in self.prefix_exempt_routes):
            return await call_next(request)
